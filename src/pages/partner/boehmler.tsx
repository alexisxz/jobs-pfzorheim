...
export default function Boehmler() {
    const [jobs, setJobs] = useState<Job[]>(fakeJobs.filter(job => job.status !== false && job.company.name === 'Böhmler GmbH'))
    const [jobCardsQuantity, setJobCardsQuantity] = useState<number>(fakeJobs.filter(job => job.status !== false && job.company.name === 'Böhmler GmbH').length)
    const [jobsList, setJobsList] = useState<Job[]>()
    const [maxJobCards, setMaxJobCards] = useState<number>(5)

    //filter
    const [filtersCategory, setFiltersCategory] = useState(formatHomeFilter(fakeJobs.filter(job => job.company.name === 'Böhmler GmbH')))
    ...
    useEffect(() => {
        const allJobs = fakeJobs.filter(job => job.status !== false && job.company.name === 'Böhmler GmbH')
    ...
    return (
        <>
            <Head>
                <title>Böhmler GmbH Jobs by stirner/stirner</title>
                <meta name="description" content="Jobs at Böhmler GmbH" />
                ...
                <section className={`${styles.section}`}>
                    <div className={styles.container}>
                        <div className={styles.picsWrapper}>
                            ...
                            <Image src={BoehmlerLogo} alt='Boehmler logo' />
                        </div>
                    </div>
                </section>
